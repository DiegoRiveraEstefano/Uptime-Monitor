defmodule UptimeMonitor.IncidentsAndAlertsTest do
  # Async false because we test async dispatch processes
  ...
  describe "incident lifecycle & alert grouping" do
    test "report_failure/2 creates an open incident on first failure", %{monitor: monitor} do
      assert {:ok, %Incident{} = incident} =
               Incidents.report_failure(monitor, "Connection Timeout")
    ...
    test "report_failure/2 groups subsequent failures under the same open incident", %{
      monitor: monitor
    } do
    ...
    test "report_recovery/1 resolves the active incident, calculates downtime, and resolves alerts",
         %{monitor: monitor} do
  ...
  describe "incident post-mortems" do
    ...
    test "create_post_mortem/3 logs markdown document attached to incident", %{
      user: user,
      incident: incident
    } do
      attrs = %{
  ...
  describe "notification channels & integrations" do
    test "create_notification_channel/2 saves settings and validates details based on type", %{
      tenant: tenant
    } do
      # Platform channel
      assert {:ok, %NotificationChannel{} = chan1} =
               Alerts.create_notification_channel(tenant, %{
                 type: "platform",
                 active: true,
                 config: %{}
               })
  ...
  describe "platform built-in alerts" do
    test "in-app notifications are logged and can be marked as read", %{tenant: tenant} do
      {:ok, notif} =
        Alerts.create_platform_notification(tenant.id, %{
          title: "Alert",
